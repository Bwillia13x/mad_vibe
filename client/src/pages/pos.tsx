...
export default function POSPage() {
  ...
  const {
    ...
    error: salesError
  } = useQuery<any[]>({ queryKey: ['/api/pos/sales'] })
  ...
  const [discountPct, setDiscountPct] = useState<number>(() => {
    try {
      const v = localStorage.getItem('pos_discount')
  ...
  const [taxPct, setTaxPct] = useState<number>(() => {
    try {
      const r = localStorage.getItem('pos_region')
  ...
  const [regionCode, setRegionCode] = useState<string>(() => {
    try {
      return localStorage.getItem('pos_region') || 'ab'
    } catch {
      return 'ab'
    }
  })
  ...
  useEffect(() => {
    try {
      localStorage.setItem('pos_discount', String(discountPct))
    } catch {}
  }, [discountPct])
  useEffect(() => {
    try {
      localStorage.setItem('pos_region', regionCode)
    } catch {}
  }, [regionCode])
  ...
  const addService = (s: Service) => {
    setCart((prev) => {
      ...
      return [
        ...prev,
        { kind: 'service', id: s.id, name: s.name, price: parseFloat(String(s.price)), quantity: 1 }
      ]
    })
  }
  ...
  const discountAmt = useMemo(
    () => (subtotal * Math.max(0, discountPct)) / 100,
    [subtotal, discountPct]
  )
  const taxAmt = useMemo(
    () => ((subtotal - discountAmt) * Math.max(0, taxPct)) / 100,
    [subtotal, discountAmt, taxPct]
  )
  ...
  const checkout = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/pos/sales', {
        ...
        body: JSON.stringify({ items, discountPct, taxPct })
      })
  ...
  return (
    <PageContainer>
      <PageHeader
        title="POS — Checkout"
        subtitle="Add services and products to cart, apply discounts and charge."
        testId="heading-pos"
        actions={
          <Button asChild variant="outline" size="sm" aria-label="Export sales CSV">
            <a href="/api/pos/sales/export" download>
              Export Sales CSV
            </a>
          </Button>
        }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <Card className="lg:col-span-2">
          ...
          <CardContent>
            {servicesError ? (
              <div className="text-sm text-red-400">Failed to load services.</div>
            ) : servicesLoading ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                {Array.from({ length: 6 }).map((_, i) => (
                  <Skeleton key={i} className="h-16 rounded-xl bg-slate-800/60" />
                ))}
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                ...
                {services.length === 0 && (
                  <div className="col-span-full text-sm text-gray-500">No services available.</div>
                )}
              </div>
            )}
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            {inventoryError ? (
              <div className="text-sm text-red-400">Failed to load inventory.</div>
            ) : (
              <div className="space-y-2">
                ...
                {inventoryLoading ? (
                  <div className="space-y-2">
                    {Array.from({ length: 6 }).map((_, i) => (
                      <Skeleton key={i} className="h-8 rounded bg-slate-800/60" />
                    ))}
                  </div>
                ) : (
      ...
      <div className="mt-4 grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <CardContent>
            {cart.length === 0 ? (
              <div className="text-sm text-gray-500">
                No items yet. Click a service or product to add.
              </div>
            ) : (
              <div className="space-y-2">
                {cart.map((i, idx) => (
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        ...
                        onClick={() =>
                          setCart((prev) =>
                            prev.map((p, ix) =>
                              ix === idx ? { ...p, quantity: Math.max(1, p.quantity - 1) } : p
                            )
                          )
                        }
                      >
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          setCart((prev) =>
                            prev.map((p, ix) => (ix === idx ? { ...p, quantity: v } : p))
                          )
                        }}
                      />
                      <Button
                        ...
                        onClick={() =>
                          setCart((prev) =>
                            prev.map((p, ix) =>
                              ix === idx ? { ...p, quantity: p.quantity + 1 } : p
                            )
                          )
                        }
                      >
                      ...
                      <Button
                        ...
                        onClick={() => setCart((prev) => prev.filter((_, ix) => ix !== idx))}
                      >
                        Remove
                      </Button>
                    </div>
                ...
                <div className="flex items-center justify-end gap-6 border-t pt-2 mt-2 text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Input
                      ...
                      value={discountPct}
                      onChange={(e) =>
                        setDiscountPct(Math.max(0, parseFloat(e.target.value || '0')))
                      }
                    />
                ...
                <div className="flex flex-col items-end mt-2 text-sm">
                  <div className="flex items-center justify-between gap-10 w-full max-w-xs">
                    <span>Subtotal</span>
                  ...
                  <div className="flex items-center justify-between gap-10 w-full max-w-xs font-semibold border-t pt-2 mt-2">
                    <span>Total</span>
                    <span>${total.toFixed(2)}</span>
                  </div>
                </div>
                <div className="mt-3">
                  <Button onClick={checkout} data-testid="button-pos-checkout">
        ...
        <Card>
          ...
          <CardContent>
            {salesError ? (
              <div className="text-sm text-red-400">Failed to load recent sales.</div>
            ) : salesLoading ? (
              <div className="space-y-2">
                {Array.from({ length: 6 }).map((_, i) => (
                  <Skeleton key={i} className="h-10 rounded bg-slate-800/60" />
                ))}
              </div>
              ...
              <>
                <div className="flex items-center justify-between mb-2 text-xs text-gray-600 dark:text-gray-400">
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      size="sm"
                      variant={salesSortBy === 'date' ? 'default' : 'outline'}
                      onClick={() => setSalesSortBy('date')}
                    >
                      Date
                    </Button>
                    <Button
                      size="sm"
                      variant={salesSortBy === 'amount' ? 'default' : 'outline'}
                      onClick={() => setSalesSortBy('amount')}
                    >
                      Amount
                    </Button>
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => setSalesSortDir((d) => (d === 'desc' ? 'asc' : 'desc'))}
                    >
                      {salesSortDir === 'desc' ? 'Desc' : 'Asc'}
                    </Button>
                  </div>
                </div>
                {(() => {
                  const items = [...sales].sort((a: any, b: any) => {
                    const va =
                      salesSortBy === 'date' ? +new Date(a.createdAt) : parseFloat(a.total || '0')
                    const vb =
                      salesSortBy === 'date' ? +new Date(b.createdAt) : parseFloat(b.total || '0')
                ...
                <div className="flex items-center justify-between mt-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          const res = await fetch(`/api/pos/sales/${lastSale.id}`, {
                            method: 'DELETE'
                          })
                    ...
                    >
                      <Undo2 className="h-4 w-4 mr-1" /> Void
                    </Button>
                  </div>
                </div>
              </div>
