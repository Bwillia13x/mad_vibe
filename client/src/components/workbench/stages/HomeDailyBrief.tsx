  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <Card className="bg-slate-900/60 border-slate-800 transition-all duration-200 ease-in-out hover:bg-slate-900/70">
          <CardHeader>
          ...
          <CardContent className="text-xs text-slate-400">
            {nextStage ? (
              <div>
                ...
                <Badge
                  variant="outline"
                  className="mt-3 border-slate-700 text-[10px] uppercase text-slate-300 transition-all duration-200 ease-in-out animate-in fade-in duration-500"
                >
        ...
        <Card className="bg-slate-900/60 border-slate-800 transition-all duration-200 ease-in-out hover:bg-slate-900/70">
          ...
          <CardContent className="space-y-2 text-xs text-slate-400">
            {stages.map((stage) => (
              <div
                key={stage.slug}
                className="flex items-center justify-between transition-all duration-200 ease-in-out hover:bg-slate-800/30 rounded-lg px-2 py-1"
              >
                <span>{stage.shortTitle}</span>
                <Badge
                  variant="outline"
                  className="border-slate-700 text-[10px] uppercase text-slate-300 transition-all duration-200 ease-in-out hover:scale-105"
                >
        ...
        <CardContent>
          {recentEntries.length === 0 ? (
            <p className="text-xs text-slate-500">
              No journal entries yet — advance a stage to populate.
            </p>
          ) : (
            <ul className="space-y-2 text-xs text-slate-400">
              {recentEntries.map((entry) => (
                <li
                  key={entry.id}
                  className="flex items-start gap-3 rounded border border-slate-800 bg-slate-950/60 px-3 py-2 transition-all duration-200 ease-in-out hover:bg-slate-950/80 hover:border-slate-700"
                >
