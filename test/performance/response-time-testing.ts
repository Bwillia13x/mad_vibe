...
export class ResponseTimeTester {
  ...
  private async testEndpointResponseTime(
    ...
    return this.runTest(testName, async () => {
      ...
      if (statistics.average > config.thresholds.maxAverageMs) {
        violations.push(`Average response time (${statistics.average.toFixed(1)}ms) exceeds threshold (${config.thresholds.maxAverageMs}ms)`);
      }
      
      if (statistics.p95 > config.thresholds.maxP95Ms) {
        violations.push(`95th percentile (${statistics.p95.toFixed(1)}ms) exceeds threshold (${config.thresholds.maxP95Ms}ms)`);
      }
      
      if (statistics.p99 > config.thresholds.maxP99Ms) {
        violations.push(`99th percentile (${statistics.p99.toFixed(1)}ms) exceeds threshold (${config.thresholds.maxP99Ms}ms)`);
      }
      
      if (statistics.max > config.thresholds.maxSingleRequestMs) {
        violations.push(`Maximum response time (${statistics.max}ms) exceeds threshold (${config.thresholds.maxSingleRequestMs}ms)`);
      }
      ...
      console.log(`    ✅ avg=${statistics.average.toFixed(1)}ms, p95=${statistics.p95.toFixed(1)}ms, p99=${statistics.p99.toFixed(1)}ms`);
  ...
  private async testConcurrentLatency(config: ResponseTimeTestConfig): Promise<TestResult> {
    return this.runTest('concurrent-latency', async () => {
      ...
      if (statistics.average > expectedMaxTime) {
        violations.push(`Concurrent average response time (${statistics.average.toFixed(1)}ms) exceeds threshold (${expectedMaxTime}ms)`);
      }
      ...
      if (throughput < minThroughput) {
        violations.push(`Concurrent throughput (${throughput.toFixed(1)} RPS) below threshold (${minThroughput} RPS)`);
      }
      ...
      console.log(`    ✅ ${config.concurrentRequests} concurrent requests: avg=${statistics.average.toFixed(1)}ms, throughput=${throughput.toFixed(1)} RPS`);
  ...
  private async testPageLoadTimes(config: PageLoadTestConfig): Promise<TestResult[]> {
    ...
    for (const page of config.pages) {
      results.push(await this.runTest(`page-load-${page.replace(/[^a-zA-Z0-9]/g, '-')}`, async () => {
        const startTime = Date.now();
        ...
        try {
          ...
          if (!result.passed) {
            throw new Error(`Page load time (${loadTime}ms) exceeds threshold (${config.maxLoadTimeMs}ms)`);
          }
          
          console.log(`    ✅ ${page}: ${loadTime}ms (${assetCount} assets, ${(content.length / 1024).toFixed(1)}KB)`);
  ...
  private async testStreamingPerformance(config: StreamingTestConfig): Promise<TestResult[]> {
    ...
    for (const message of config.testMessages) {
      results.push(await this.runTest(`streaming-${message.substring(0, 20).replace(/[^a-zA-Z0-9]/g, '-')}`, async () => {
        const streamingResult = await this.measureStreamingResponse(config.endpoint, message, config);
        
        if (!streamingResult.passed) {
          throw new Error(`Streaming test failed: ${streamingResult.violations.join(', ')}`);
        }
        
        console.log(`    ✅ "${message.substring(0, 30)}...": first byte=${streamingResult.firstByteTime}ms, rate=${streamingResult.streamingRate.toFixed(1)} chars/s`);
        ...
        for (const line of lines) {
          ...
          if (data === '[DONE]') {
            ...
            if (firstByteTime > config.maxFirstByteMs) {
              violations.push(`First byte time (${firstByteTime}ms) exceeds threshold (${config.maxFirstByteMs}ms)`);
            }
            
            if (streamingRate < config.minStreamingRateCharsPerSec) {
              violations.push(`Streaming rate (${streamingRate.toFixed(1)} chars/s) below threshold (${config.minStreamingRateCharsPerSec} chars/s)`);
            }
            
            if (totalTime > config.maxTotalTimeMs) {
              violations.push(`Total streaming time (${totalTime}ms) exceeds threshold (${config.maxTotalTimeMs}ms)`);
            }
  ...
  private calculateStatistics(measurements: ResponseTimeMeasurement[]): ResponseTimeStatistics {
    ...
    const variance = count > 0 
      ? responseTimes.reduce((acc, rt) => acc + Math.pow(rt - average, 2), 0) / count 
      : 0;
...
export function createDefaultResponseTimeConfig(baseUrl: string): ResponseTimeTestConfig {
  return {
    ...
    pageLoadTest: {
      enabled: true,
      pages: ['/', '/pos', '/analytics', '/inventory', '/marketing', '/loyalty', '/scheduling', '/staff'],
      maxLoadTimeMs: 2000,
      includeAssets: true
    },
    streamingTest: {
