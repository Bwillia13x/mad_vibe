...
export class ConnectionResourceTester {
  ...
  async testConnectionPooling(): Promise<ConnectionTestResult> {
    console.log(`Testing connection pooling with ${this.config.concurrentUsers} concurrent users...`);
    ...
    this.results.connection.failedRequests = this.results.connection.totalRequests - this.results.connection.successfulRequests;
    this.results.connection.averageResponseTime = responseTimes.length > 0 
    ...
    this.results.connection.requestsPerSecond = (this.results.connection.totalRequests / totalDuration) * 1000;
    this.results.connection.errorRate = (this.results.connection.failedRequests / this.results.connection.totalRequests) * 100;
  ...
  async testResourceManagement(): Promise<ResourceTestResult> {
    ...
    this.results.resource.memoryGrowth = this.results.resource.finalMemory - this.results.resource.initialMemory;
  ...
  async runComprehensiveTest(): Promise<{
    ...
    try {
      ...
      if (connectionResult.averageResponseTime > 2000) {
        issues.push(`High average response time: ${connectionResult.averageResponseTime.toFixed(0)}ms`);
      }
