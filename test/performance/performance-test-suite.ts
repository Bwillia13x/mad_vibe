  ...
  private async executeLoadTests(): Promise<TestResult[]> {
    ...
    results.push(
      await this.runTest('load-test-ramp-up', async () => {
        ...
        console.log(
          `✅ Load test passed: ${result.summary.totalRequests} requests, ` +
    ...
    results.push(
      await this.runTest('load-test-spike', async () => {
        ...
        console.log(
          `✅ Spike test completed: ${result.summary.totalRequests} requests, ` +
            `${result.summary.averageResponseTime.toFixed(0)}ms avg response time`
        )
    ...
    results.push(
      await this.runTest('load-test-sustained', async () => {
        ...
        console.log(
          `✅ Sustained load test passed: ${result.summary.totalRequests} requests over ${result.summary.duration}ms`
        )
