...
export class AccessibilityTests {
  ...
  private async waitForLoadState(state: 'load' | 'domcontentloaded' | 'networkidle'): Promise<void> {
    if (!this.page) return;
    switch (state) {
      case 'load':
  ...
  private async testPageAccessibility(pageName: string, path: string): Promise<AccessibilityTestResult> {
    ...
    try {
      ...
      const criticalViolations = axeResults.violations.filter((v: any) => v.impact === 'critical').length;
      const seriousViolations = axeResults.violations.filter((v: any) => v.impact === 'serious').length;
