...
export class LoyaltyWorkflowTestFramework {
    ...
    try {
      ...
      await this.page.waitForFunction(
        () => {
          ...
          return (
            status &&
            (status.textContent?.includes('Success') || status.textContent?.includes('Error'))
          )
        },
        { timeout: 10000 }
      )
  ...
  private async testRewardAdditionWorkflow(): Promise<LoyaltyWorkflowTestResult> {
    ...
    try {
      ...
      await this.page.waitForFunction(
        ...
        `[data-testid^="add-reward-"]:first-child`,
        { timeout: 5000 }
      )
  ...
  private async testPointsCalculationAccuracy(): Promise<LoyaltyWorkflowTestResult> {
    ...
    try {
      ...
      const specificCustomer = this.page
        .locator('[data-testid^="customer-filter-"]:not([data-testid="customer-filter-all"])')
        .first()
      if ((await specificCustomer.count()) > 0) {
        ...
        if (updatedText === calculationText) {
          ...
          if (!updatedText?.includes('(') || !updatedText?.includes(')')) {
            throw new Error(
              'Customer name should be shown in calculation when specific customer selected'
            )
          }
        }
      }
  ...
  private async testCrossModuleWorkflowConsistency(): Promise<LoyaltyWorkflowTestResult> {
    ...
    try {
      ...
      const customerOptions = await this.page
        .locator(
          '[data-testid^="customer-select-"]:not([data-testid="customer-select-placeholder"])'
        )
        .count()
