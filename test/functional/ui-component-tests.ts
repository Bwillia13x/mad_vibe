...
export class UIComponentTestSuite {
  ...
  private async testSidebarNavigation(): Promise<void> {
    await this.runTest('Sidebar Navigation', async () => {
      ...
      const navigationTests = [
        { name: 'POS', testId: 'button-tool-pos', expectedUrl: '/pos', expectedHeading: 'POS — Checkout' },
        { name: 'Analytics', testId: 'button-tool-analytics', expectedUrl: '/analytics', expectedHeading: 'Performance Analytics' },
        { name: 'Scheduling', testId: 'button-tool-scheduling', expectedUrl: '/scheduling', expectedHeading: 'Scheduling' },
        { name: 'Inventory', testId: 'button-tool-inventory', expectedUrl: '/inventory', expectedHeading: 'Inventory Management' },
        { name: 'Staff', testId: 'button-tool-staff', expectedUrl: '/staff', expectedHeading: 'Staff Management' },
        { name: 'Marketing', testId: 'button-tool-marketing', expectedUrl: '/marketing', expectedHeading: 'Marketing' },
        { name: 'Loyalty', testId: 'button-tool-loyalty', expectedUrl: '/loyalty', expectedHeading: 'Loyalty Program' },
        { name: 'Chat', testId: 'button-tool-chat', expectedUrl: '/', expectedHeading: 'Andreas Vibe Business Management' }
      ];
      ...
      for (const nav of navigationTests) {
        try {
          ...
          if (!currentUrl.endsWith(nav.expectedUrl)) {
            throw new Error(`Navigation to ${nav.name} failed: expected ${nav.expectedUrl}, got ${currentUrl}`);
          }
          ...
          if (!headingText?.includes(nav.expectedHeading.split(' ')[0])) {
            console.warn(`Heading mismatch for ${nav.name}: expected to contain "${nav.expectedHeading.split(' ')[0]}", got "${headingText}"`);
          }
      ...
      if (failedNavigations.length > 0) {
        throw new Error(`Navigation failures: ${failedNavigations.map(f => `${f.page}: ${f.error}`).join('; ')}`);
      }

      return { navigationTests: results.length, successful: results.filter(r => r.success).length };
    });
  }
  ...
  private async testChatInterface(): Promise<void> {
    await this.runTest('Chat Interface Functionality', async () => {
      ...
      const initialValue = await this.page.$eval(messageInput, el => (el as HTMLInputElement).value);
      if (initialValue !== '') {
        throw new Error(`Message input should be empty initially, got: ${initialValue}`);
      }

      // Verify send button is initially disabled
      const initialDisabled = await this.page.$eval(sendButton, el => (el as HTMLButtonElement).disabled);
      if (!initialDisabled) {
      ...
      const enabledState = await this.page.$eval(sendButton, el => (el as HTMLButtonElement).disabled);
      if (enabledState) {
      ...
      const promptValue = await this.page.$eval(messageInput, el => (el as HTMLInputElement).value);
      if (!promptValue.includes('schedule')) {
        throw new Error(`Prompt button should populate input with schedule-related text, got: ${promptValue}`);
      }
  ...
  private async testAnalyticsInterface(): Promise<void> {
    await this.runTest('Analytics Interface Functionality', async () => {
      ...
      await this.page.goto(`${this.testEnvironment.baseUrl}/analytics`, { waitUntil: 'networkidle0' });
      ...
      const heading = await this.page.$eval('[data-testid="heading-analytics"]', el => el.textContent);
      ...
      const chartSelectors = [
        ...
        '[data-testid="appointments-chart"]'
      ];
  ...
  private async testFormValidation(): Promise<void> {
    await this.runTest('Form Validation', async () => {
      ...
      const sendButton = '[data-testid="button-send"]';
      const isDisabled = await this.page.$eval(sendButton, el => (el as HTMLButtonElement).disabled);
      ...
      const isEnabled = await this.page.$eval(sendButton, el => !(el as HTMLButtonElement).disabled);
      ...
      const isDisabledAgain = await this.page.$eval(sendButton, el => (el as HTMLButtonElement).disabled);
  ...
  private async testAccessibility(): Promise<void> {
    await this.runTest('Accessibility Features', async () => {
      ...
      const unlabeledElements = interactiveElements.filter(el => 
        !el.hasAriaLabel && !el.hasAriaLabelledBy && !el.textContent && !el.hasTitle
      );
