...
import type { TestHttpClient, TestDataManager, PerformanceMonitor } from '../utils/test-environment.js';
...
export class BusinessLogicTestSuite {
  ...
  private async testPosTransactionCalculations(): Promise<void> {
    ...
    await this.runTest('Multiple Items Calculation', async () => {
      ...
      if (Math.abs(actualSubtotal - expectedSubtotal) > 0.01) {
        throw new Error(`Multiple items subtotal mismatch: expected ${expectedSubtotal}, got ${actualSubtotal}`);
      }
    ...
    await this.runTest('Discount Calculation', async () => {
      ...
      if (Math.abs(actualSubtotal - expectedSubtotal) > 0.01) {
        throw new Error(`Discount test subtotal mismatch: expected ${expectedSubtotal}, got ${actualSubtotal}`);
      }

      if (Math.abs(actualDiscount - expectedDiscount) > 0.01) {
        throw new Error(`Discount amount mismatch: expected ${expectedDiscount}, got ${actualDiscount}`);
      }

      if (Math.abs(actualTotal - expectedTotal) > 0.01) {
        throw new Error(`Discount test total mismatch: expected ${expectedTotal}, got ${actualTotal}`);
      }
    ...
    await this.runTest('Combined Discount and Tax Calculation', async () => {
      ...
      if (Math.abs(actualDiscount - expectedDiscount) > 0.01) {
        throw new Error(`Combined test discount mismatch: expected ${expectedDiscount}, got ${actualDiscount}`);
      }
      ...
      if (Math.abs(actualTotal - expectedTotal) > 0.01) {
        throw new Error(`Combined test total mismatch: expected ${expectedTotal}, got ${actualTotal}`);
      }
  ...
  private async testInventoryManagement(): Promise<void> {
    ...
    await this.runTest('Inventory Status Calculation', async () => {
      ...
      for (const item of inventory) {
        ...
        } else if (item.currentStock > 0 && item.currentStock <= item.minStock && item.status !== 'low-stock') {
          validationErrors.push(`Item ${item.name}: currentStock (${item.currentStock}) <= minStock (${item.minStock}) but status is ${item.status}`);
    ...
    await this.runTest('Low Inventory Scenario Validation', async () => {
      ...
      if (outOfStockItems.length === 0 && lowStockItems.length === 0) {
        throw new Error('Low inventory scenario should produce some out-of-stock or low-stock items');
      }
    ...
    await this.runTest('Product Sale Inventory Deduction', async () => {
      ...
      const inventory = await this.httpClient.getJson('/api/inventory');
      const inStockProducts = inventory.filter((item: any) => 
      ...
      const sale = await this.httpClient.postJson('/api/pos/sales', {
        items: [
          { kind: 'product', id: product.id, quantity: saleQuantity }
        ]
      });
      ...
      if (updatedProduct.currentStock !== expectedStock) {
        throw new Error(`Stock deduction failed: expected ${expectedStock}, got ${updatedProduct.currentStock}`);
      }
      ...
      if (restoredProduct.currentStock !== initialStock) {
        throw new Error(`Stock restoration failed: expected ${initialStock}, got ${restoredProduct.currentStock}`);
      }
  ...
  private async testSchedulingLogic(): Promise<void> {
    ...
    await this.runTest('Appointment Time Validation', async () => {
      ...
      for (const appointment of appointments) {
        ...
        if (startTime >= endTime) {
          validationErrors.push(`Appointment ${appointment.id}: start time (${startTime}) is not before end time (${endTime})`);
        }
        ...
        if (durationMinutes < 5 || durationMinutes > 240) {
          validationErrors.push(`Appointment ${appointment.id}: duration (${durationMinutes} minutes) is unreasonable`);
        }
      }
    ...
    await this.runTest('Staff Assignment Validation', async () => {
      ...
      for (const appointment of appointments) {
        if (!staffIds.has(appointment.staffId)) {
          validationErrors.push(`Appointment ${appointment.id}: assigned to non-existent staff ${appointment.staffId}`);
        }
      }
    ...
    await this.runTest('Service Assignment Validation', async () => {
      ...
      for (const appointment of appointments) {
        if (!serviceIds.has(appointment.serviceId)) {
          validationErrors.push(`Appointment ${appointment.id}: assigned to non-existent service ${appointment.serviceId}`);
        }
      }
      ...
      for (const appointment of busyAppointments) {
        ...
        for (const existing of schedule) {
          if ((start < existing.end && end > existing.start)) {
            throw new Error(`Scheduling conflict detected for staff ${staffId}: ${start} - ${end} overlaps with ${existing.start} - ${existing.end}`);
          }
        }
    ...
    await this.runTest('Appointment Gaps Scenario Validation', async () => {
      ...
      if (gapAppointments.length > 1) {
        const sortedAppointments = gapAppointments.sort((a: any, b: any) => 
  ...
  private async testLoyaltyProgram(): Promise<void> {
    ...
    await this.runTest('Loyalty Entry Creation', async () => {
      ...
      if (loyaltyEntry.customerId !== customer.id) {
        throw new Error(`Customer ID mismatch: expected ${customer.id}, got ${loyaltyEntry.customerId}`);
      }
    ...
    await this.runTest('Customer Loyalty Filtering', async () => {
      ...
      const customerEntries = await this.httpClient.getJson(`/api/loyalty/entries?customerId=${customer.id}`);

      // Verify filtering works
      const expectedCount = entries.length;
      if (customerEntries.length < expectedCount) {
        throw new Error(`Customer filtering failed: expected at least ${expectedCount} entries, got ${customerEntries.length}`);
      }

      // Verify all returned entries belong to the customer
      const invalidEntries = customerEntries.filter((entry: any) => entry.customerId !== customer.id);
      if (invalidEntries.length > 0) {
        throw new Error(`Customer filtering returned entries for other customers: ${invalidEntries.length} invalid entries`);
      }
    ...
    await this.runTest('Invalid Loyalty Entry Validation', async () => {
      ...
      for (const scenario of invalidScenarios) {
        try {
          ...
          results.push({ scenario: scenario.name, success: false, error: 'Should have failed but succeeded' });
        } catch (error) {
  ...
  private async testMarketingCampaigns(): Promise<void> {
    ...
    await this.runTest('Campaign Creation and Status Management', async () => {
      ...
      const updateResponse = await this.httpClient.patch(`/api/marketing/campaigns/${campaign.id}`, {
        status: 'active'
      });
    ...
    await this.runTest('Marketing Performance Calculation', async () => {
      ...
      const calculatedImpressions = campaigns.reduce((sum: number, c: any) => sum + c.impressions, 0);
      const calculatedClicks = campaigns.reduce((sum: number, c: any) => sum + c.clicks, 0);
      const calculatedConversions = campaigns.reduce((sum: number, c: any) => sum + c.conversions, 0);

      if (Math.abs(summary.impressions - calculatedImpressions) > 1) {
        throw new Error(`Summary impressions mismatch: expected ${calculatedImpressions}, got ${summary.impressions}`);
      }

      if (Math.abs(summary.clicks - calculatedClicks) > 1) {
        throw new Error(`Summary clicks mismatch: expected ${calculatedClicks}, got ${summary.clicks}`);
      }

      if (Math.abs(summary.conversions - calculatedConversions) > 1) {
        throw new Error(`Summary conversions mismatch: expected ${calculatedConversions}, got ${summary.conversions}`);
      }

      // Validate CTR calculation
      const expectedCTR = calculatedImpressions > 0 ? (calculatedClicks / calculatedImpressions * 100) : 0;
      if (Math.abs(summary.ctr - expectedCTR) > 0.1) {
        throw new Error(`Summary CTR mismatch: expected ${expectedCTR.toFixed(2)}, got ${summary.ctr}`);
      }

      // Validate conversion rate calculation
      const expectedConvRate = calculatedClicks > 0 ? (calculatedConversions / calculatedClicks * 100) : 0;
      if (Math.abs(summary.convRate - expectedConvRate) > 0.1) {
        throw new Error(`Summary conversion rate mismatch: expected ${expectedConvRate.toFixed(2)}, got ${summary.convRate}`);
      }
