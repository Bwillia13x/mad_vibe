...
export class FunctionalTestSuite {
  ...
  constructor(
    ...
    this.businessLogicSuite = new BusinessLogicTestSuite(httpClient, dataManager, performanceMonitor);
  ...
  async runAllTests(options: {
    ...
    if (includeBusinessLogic) {
      ...
      try {
        ...
        businessLogicResults = [{ testName: 'Business Logic Suite', status: 'fail', error: String(error) }];
      }
    }
  ...
  private printFinalSummary(result: FunctionalTestSuiteResult): void {
    ...
    if (result.apiTestResults) {
      ...
      if (result.apiTestResults.summary) {
        ...
        console.log(`   Avg Response: ${result.apiTestResults.summary.averageResponseTime.toFixed(2)}ms`);
      } else {
    ...
    if (result.summary.failed > 0) {
      ...
      // API failures
      if (result.apiTestResults && result.apiTestResults.summary && result.apiTestResults.summary.failed > 0) {
