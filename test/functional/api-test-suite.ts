...
import { createApiEndpointTests, createIndividualResourceTests, createErrorScenarioTests } from './api-endpoint-tests.js';
import type { TestHttpClient, PerformanceMonitor, TestDataManager } from '../utils/test-environment.js';
...
export class ApiTestSuite {
  ...
  private async validateCsvExports(): Promise<void> {
    const csvEndpoints = [
      { path: '/api/analytics/export', name: 'Analytics CSV', expectedHeaders: ['date', 'totalRevenue'] },
      { path: '/api/pos/sales/export', name: 'POS Sales CSV', expectedHeaders: ['id', 'createdAt'] },
      { path: '/api/loyalty/entries/export', name: 'Loyalty CSV', expectedHeaders: ['id', 'customerId'] }
    ];
  ...
  private async testPosSaleCrud(): Promise<void> {
    try {
      ...
      const newSale = await this.httpClient.postJson('/api/pos/sales', {
        items: [
          { kind: 'service', name: 'Test Service', quantity: 1, unitPrice: 50 }
        ],
        discountPct: 10,
        taxPct: 8.5
      });
  ...
  private async testMarketingCampaignCrud(): Promise<void> {
    try {
      ...
      // Update campaign
      const updateResponse = await this.httpClient.patch(`/api/marketing/campaigns/${newCampaign.id}`, {
        status: 'active',
  ...
  private async testLoyaltyEntryCrud(): Promise<void> {
    try {
      ...
      const customerEntries = await this.httpClient.getJson(`/api/loyalty/entries?customerId=${customers[0].id}`);
      const foundEntry = Array.isArray(customerEntries) && 
        customerEntries.some(entry => entry.id === newEntry.id);
