  ...
  private async runComprehensiveBypassTests(): Promise<void> {
    ...
    try {
      ...
      if (failedTests.length > 0) {
        throw new Error(`Authentication bypass tests failed: ${failedTests.map(t => t.testName).join(', ')}`);
      }
  ...
  private async testAdminTokenValidation(): Promise<void> {
    ...
    try {
      ...
      if (noTokenResponse.status !== 200) {
        throw new Error(`Health endpoint should be accessible without token, got ${noTokenResponse.status}`);
      }
  ...
  private async testAuthenticationBypass(): Promise<void> {
    ...
    try {
      ...
      if (unauthorizedAttempts.length > 0) {
        throw new Error(`Authentication bypass detected: ${unauthorizedAttempts.length} unauthorized requests succeeded`);
      }

      if (serverErrors.length > 0) {
        throw new Error(`Server errors during bypass attempts: ${serverErrors.length} requests caused server errors`);
      }
