...
export class APISecurityTests {
  ...
  private async testEnvironmentVariableSecurity(): Promise<void> {
    ...
    try {
      ...
      if (unsafeTests.length > 0) {
        throw new Error(
          `Environment variable security issues detected: ${unsafeTests.length} failures`
        )
      }
  ...
  private async testAPIEndpointSecurity(): Promise<void> {
    ...
    try {
      ...
      const securityIssues = testResults.filter(
        (r) => r.exposesStackTrace || r.exposesFilePaths || (r.status === 0 && r.error)
      )
  ...
  private async testHTTPMethodSecurity(): Promise<void> {
    ...
    try {
      ...
      const testEndpoints = ['/api/health', '/api/profile', '/api/services']
      ...
      const dangerousMethodResults = testResults.filter(
        (r) => dangerousMethods.includes(r.method) && r.allowed
      )
