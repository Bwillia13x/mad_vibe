...
import { AuthenticationSecurityTests, AuthenticationTestResult } from './authentication-security-tests';
...
import { EnvironmentSecurityTests, EnvironmentSecurityTestResult } from './environment-security-tests';
...
export class SecurityTestSuite {
  ...
  async runAllSecurityTests(): Promise<SecurityTestSuiteResult> {
    ...
    const summary = this.analyzeSecurity(authenticationResults, inputValidationResults, apiSecurityResults, environmentSecurityResults);
...
${results.authenticationResults.map(r => 
  `- ${r.status === 'pass' ? '✅' : r.status === 'fail' ? '❌' : '⏭️'} ${r.testName} (${r.duration}ms)`
).join('\n')}

### Input Validation & Injection Prevention
${results.inputValidationResults.map(r => 
  `- ${r.status === 'pass' ? '✅' : r.status === 'fail' ? '❌' : '⏭️'} ${r.testName} (${r.duration}ms)`
).join('\n')}

### API Security Validation
${results.apiSecurityResults.map(r => 
  `- ${r.status === 'pass' ? '✅' : r.status === 'fail' ? '❌' : '⏭️'} ${r.testName} (${r.duration}ms)`
).join('\n')}

### Environment Variable Security
${results.environmentSecurityResults.map(r => 
  `- ${r.status === 'pass' ? '✅' : r.status === 'fail' ? '❌' : '⏭️'} ${r.testName} (${r.duration}ms)`
).join('\n')}
...
${[...results.authenticationResults, ...results.inputValidationResults, ...results.apiSecurityResults, ...results.environmentSecurityResults]
