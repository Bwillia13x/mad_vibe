...
export class AuthenticationBypassTests {
  ...
  private async testHeaderInjectionPrevention(): Promise<void> {
    ...
    try {
      ...
      if (!allBlocked) {
        throw new Error(
          `Header injection not properly blocked: ${results.filter((r) => !r.blocked).length} attempts succeeded`
        )
      }
  ...
  private async testTokenFormatValidation(): Promise<void> {
    ...
    try {
      ...
      if (!allBlocked) {
        throw new Error(
          `Malicious tokens not properly blocked: ${results.filter((r) => !r.blocked).length} attempts succeeded`
        )
      }
    ...
    try {
      ...
      if (rateLimitingDisabled) {
        ...
        if (unauthorizedAttempts.length !== results.length) {
          throw new Error(
            `Expected all requests to be rejected with 401 when rate limiting is disabled, but got mixed responses`
          )
        }
      } else {
  ...
  private async testSessionSecurityValidation(): Promise<void> {
    ...
    try {
      ...
      if (!allTestsPassed) {
        throw new Error(
          `Session security validation failed: ${sessionTests
            .filter((t) => !t.passed)
            .map((t) => t.test)
            .join(', ')}`
        )
  ...
  private async testAdvancedBypassAttempts(): Promise<void> {
    ...
    try {
      const bypassAttempts = [
        ...
        // Multiple headers
        {
          headers: { Authorization: ['Bearer fake', 'Bearer admin'] },
        ...
        {
          headers: { Authorization: 'Bearer \\x61\\x64\\x6D\\x69\\x6E' },
        ...
        // Null byte injection
        {
          headers: { Authorization: 'Bearer admin\x00bypass' },
        ...
        // Command injection
        {
          headers: { Authorization: 'Bearer admin`whoami`' },
        ...
        // NoSQL injection
        {
          headers: { Authorization: "Bearer admin'; return true; //" },
      ...
      if (unblockedAttempts.length > 0) {
        throw new Error(
          `Advanced bypass attempts not blocked: ${unblockedAttempts.map((a) => a.description).join(', ')}`
        )
      }
