  ...
  private async testSQLInjectionPrevention(): Promise<void> {
    ...
    try {
      ...
      for (const payload of sqlPayloads) {
        try {
          const response = await this.testEnv.makeRequest(`/api/staff/${encodeURIComponent(payload)}`);
          testResults.push({
            payload,
            location: 'staff_id',
        ...
        try {
          const response = await this.testEnv.makeRequest(`/api/services/${encodeURIComponent(payload)}`);
          testResults.push({
            payload,
            location: 'service_id',
            status: response.status,
      ...
      for (let i = 0; i < Math.min(3, sqlPayloads.length); i++) {
        ...
        try {
          const response = await this.testEnv.makeRequest(`/api/appointments?date=${encodeURIComponent(payload)}`);
          testResults.push({
            payload,
            location: 'date_query',
            status: response.status,
      ...
      if (unsafeResponses.length > 0) {
        throw new Error(`SQL injection payloads caused unsafe responses: ${unsafeResponses.length} failures`);
      }
    ...
    try {
      ...
      for (const payload of maliciousPayloads) {
        ...
        const endpoints = [
          ...
          { path: '/api/pos/sales', method: 'POST', body: { items: [{ name: payload, quantity: 1, unitPrice: 10 }] } }
        ];
        ...
        for (const endpoint of endpoints) {
          try {
            ...
            if (endpoint.method === 'GET') {
              response = await this.testEnv.makeRequest(`${endpoint.path}?${endpoint.param}=${encodeURIComponent(payload)}`);
            } else {
  ...
  private async testFileUploadSecurity(): Promise<void> {
    ...
    try {
      ...
      if (existingEndpoints.length > 0) {
        console.warn(`Found unexpected file upload endpoints: ${existingEndpoints.map(e => e.endpoint).join(', ')}`);
      }
