...
export class UATTestSuite {
  ...
  async runCompleteUATSuite(): Promise<UATTestResult> {
    ...
    try {
      ...
      console.log(
        `   ✅ Scenarios Passed: ${demoScenarioSummary.passed}/${demoScenarioSummary.totalScenarios}`
      )
      console.log(
        `   ❌ Scenarios Failed: ${demoScenarioSummary.failed}/${demoScenarioSummary.totalScenarios}`
      )
      ...
      console.log(
        `   ✅ Script Steps Passed: ${demoScriptSummary.passed}/${demoScriptSummary.totalSteps}`
      )
      console.log(
        `   ❌ Script Steps Failed: ${demoScriptSummary.failed}/${demoScriptSummary.totalSteps}`
      )
      ...
      console.log(
        `   ✅ Workflows Passed: ${userWorkflowSummary.passed}/${userWorkflowSummary.totalWorkflows}`
      )
      console.log(
        `   ❌ Workflows Failed: ${userWorkflowSummary.failed}/${userWorkflowSummary.totalWorkflows}`
      )
      ...
      console.log(
        `   ✅ Shortcuts Passed: ${keyboardSummary.passed}/${keyboardSummary.totalShortcuts}`
      )
      console.log(
        `   ❌ Shortcuts Failed: ${keyboardSummary.failed}/${keyboardSummary.totalShortcuts}`
      )

      // Calculate overall summary
      const totalTests =
        demoScenarioSummary.totalScenarios +
        demoScriptSummary.totalSteps +
        userWorkflowSummary.totalWorkflows +
        keyboardSummary.totalShortcuts
      const totalPassed =
        demoScenarioSummary.passed +
        demoScriptSummary.passed +
        userWorkflowSummary.passed +
        keyboardSummary.passed
      const totalFailed =
        demoScenarioSummary.failed +
        demoScriptSummary.failed +
        userWorkflowSummary.failed +
        keyboardSummary.failed
  ...
  private generateHTMLReport(result: UATTestResult): string {
    ...
    const statusColor =
      result.summary.successRate >= 90
        ? '#22c55e'
        : result.summary.successRate >= 70
          ? '#f59e0b'
          : '#ef4444'
    ...
    <div class="container">
        ...
        <div class="section">
            ...
            <div class="test-item ${result.demoScript.result.status === 'pass' ? 'test-pass' : 'test-fail'}">
                ...
                <div class="validation-list">
                    ${result.demoScript.result.validations
                      .map(
                        (v) => `
                        <div class="validation-item">
                            <span class="status-${v.status}">${v.status === 'pass' ? '✅' : '❌'}</span>
        ...
        <div class="section">
            ...
            <div class="test-item ${result.keyboardShortcuts.result.status === 'pass' ? 'test-pass' : 'test-fail'}">
                ...
                ${result.keyboardShortcuts.result.workflow
                  .map(
                    (step) => `
                    <div class="workflow-step">
                        <span class="status-${step.status}">${step.status === 'pass' ? '✅' : '❌'}</span>
...
${result.demoScenarios.results
  ...
  .map((r) => `- **Demo Scenario:** ${r.name} - ${r.error || 'Multiple validation failures'}`)
  .join('\n')}
${result.demoScript.result.status === 'fail' ? `- **Demo Script:** ${result.demoScript.result.error || 'Script execution failed'}` : ''}
${result.userWorkflows.results
  .filter((r) => r.status === 'fail')
  .map((r) => `- **User Workflow:** ${r.name} - ${r.error || 'Workflow steps failed'}`)
  .join('\n')}
...
${
  result.summary.successRate >= 95
    ? '✅ **Excellent:** All tests passing. Platform is ready for client handoff.'
    : result.summary.successRate >= 85
      ? '⚠️ **Good:** Minor issues detected. Review failed tests before handoff.'
      : result.summary.successRate >= 70
        ? '🔧 **Needs Work:** Several issues detected. Address failures before handoff.'
