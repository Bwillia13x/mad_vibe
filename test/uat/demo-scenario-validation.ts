...
export class DemoScenarioValidation {
  ...
  private readonly scenarios = [
    ...
    { name: 'appointment_gaps', seed: 456, description: 'Scheduling gaps scenario with reduced appointments' }
  ];
  ...
  private readonly demoScriptSteps: DemoScriptStep[] = [
    ...
    },
    {
      ...
      actions: [
        ...
        { type: 'type', selector: 'textarea, input[type="text"]', value: 'Show me today\'s schedule and any gaps.' },
    ...
    },
    {
      ...
      duration: 45000,
      validations: ['pos_interface_loaded', 'cart_functionality', 'checkout_process', 'receipt_generated'],
      actions: [
    ...
    },
    {
      ...
      duration: 45000,
      validations: ['marketing_interface_loaded', 'campaign_creation', 'campaign_activation', 'metrics_display'],
      actions: [
        { type: 'navigate', value: '/marketing' },
        { type: 'wait', selector: '[data-testid="heading-marketing"]', timeout: 5000 },
        { type: 'verify', selector: 'button:has-text("Create Campaign"), button[aria-label*="Create"]' },
    ...
    },
    {
      ...
      duration: 45000,
      validations: ['loyalty_interface_loaded', 'reward_addition', 'points_addition', 'totals_calculation'],
      actions: [
        { type: 'navigate', value: '/loyalty' },
        { type: 'wait', selector: '[data-testid="heading-loyalty"]', timeout: 5000 },
        { type: 'verify', selector: 'button:has-text("Add Reward"), button[aria-label*="Add Reward"]' },
    ...
    },
    {
      ...
      duration: 30000,
      validations: ['scheduling_appointments_visible', 'inventory_status_display', 'scenario_effects_visible'],
      actions: [
  ...
  async runDemoScriptValidation(scenario: string = 'default', seed?: number): Promise<DemoScenarioResult> {
  ...
  private async testScenario(scenario: string, seed?: number, description?: string): Promise<DemoScenarioResult> {
  ...
  private async validateScenarioSetup(scenario: string): Promise<DemoScenarioResult['validations'][0]> {
    const startTime = Date.now();
    ...
    try {
      ...
      return {
        ...
        error: expectedScenario ? undefined : `Expected scenario '${scenario}', got '${healthResponse.scenario}'`
      };
  ...
  private async validateNavigation(): Promise<DemoScenarioResult['validations'][0]> {
    ...
    try {
      ...
      const modules = [
        { name: 'Chat', selector: '[data-testid="button-tool-chat"]', heading: '[data-testid="heading-main"]' },
        { name: 'POS', selector: '[data-testid="button-tool-pos"]', heading: '[data-testid="heading-pos"]' },
        { name: 'Scheduling', selector: '[data-testid="button-tool-scheduling"]', heading: '[data-testid="heading-scheduling"]' },
        { name: 'Inventory', selector: '[data-testid="button-tool-inventory"]', heading: '[data-testid="heading-inventory"]' },
        { name: 'Analytics', selector: '[data-testid="button-tool-analytics"]', heading: '[data-testid="heading-analytics"]' }
      ];
      ...
      return {
        ...
        error: successfulNavigations < totalModules ? 
          `Only ${successfulNavigations}/${totalModules} modules navigated successfully` : undefined
      };
  ...
  private async validateScenarioEffects(scenario: string): Promise<DemoScenarioResult['validations'][0]> {
    const startTime = Date.now();
    ...
    try {
      ...
      switch (scenario) {
        ...
        case 'low_inventory':
          ...
          const lowStockItems = await this.page.$$('[data-status="low-stock"], .status-low-stock, .text-red-600');
          effects.push({
        ...
        case 'appointment_gaps':
          ...
          await this.page.goto(`${this.baseUrl}/scheduling`);
          await this.page.waitForSelector('[data-testid="heading-scheduling"]', { timeout: 5000 });
          const gapAppointments = await this.page.$$('[data-testid="appointment"], .appointment-item');
          effects.push({
        ...
        case 'default':
          ...
          await this.page.goto(`${this.baseUrl}/scheduling`);
          await this.page.waitForSelector('[data-testid="heading-scheduling"]', { timeout: 5000 });
          const defaultAppointments = await this.page.$$('[data-testid="appointment"], .appointment-item');
          effects.push({
      ...
      return {
        ...
        error: successfulEffects < totalEffects ? 
          `Only ${successfulEffects}/${totalEffects} scenario effects validated` : undefined
      };
  ...
  private async validateDataConsistency(scenario: string): Promise<DemoScenarioResult['validations'][0]> {
    const startTime = Date.now();
    ...
    try {
      ...
      return {
        ...
        error: successfulChecks < totalChecks ? 
          `Only ${successfulChecks}/${totalChecks} consistency checks passed` : undefined
      };
  ...
  private async executeScriptStep(step: DemoScriptStep): Promise<DemoScenarioResult['validations'][0]> {
    const startTime = Date.now();
    ...
    try {
      ...
      return {
        ...
        error: successfulValidations < totalValidations ? 
          `Only ${successfulValidations}/${totalValidations} validations passed for ${step.name}` : undefined
      };
  ...
  private async validateStepCompletion(validation: string): Promise<{ name: string; success: boolean; error?: string }> {
    if (!this.page) throw new Error('Page not initialized');
    ...
    try {
      switch (validation) {
        ...
        case 'campaign_creation':
          const createButton = await this.page.$('button:has-text("Create"), button[aria-label*="Create"]');
