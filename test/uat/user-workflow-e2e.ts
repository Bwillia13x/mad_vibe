...
export class UserWorkflowE2E {
      ...
      steps: [
        {
          ...
          validations: [
            {
              name: 'Appointments visible',
              selector: '[data-testid="appointment"], .appointment-item',
              type: 'exists'
            },
            {
              name: 'Schedule loaded',
              selector: '[data-testid="heading-scheduling"]',
              type: 'exists'
            }
          ]
        },
        {
          ...
          validations: [
            {
              name: 'Service added to cart',
              selector: '.cart-item, [data-testid="cart-item"]',
              type: 'exists'
            },
            {
              name: 'Receipt generated',
              selector: '.receipt, [data-testid="receipt"]',
              type: 'exists'
            }
          ]
        },
        {
          ...
          actions: [
            ...
            {
              type: 'click',
              selector: '#loyalty-customer-select, [data-testid="customer-select"]'
            },
            { type: 'wait', timeout: 1000 },
          ...
          validations: [
            ...
            {
              name: 'Points input available',
              selector: '#loyalty-points-input, input[type="number"]',
              type: 'exists'
            }
          ]
        },
        {
          ...
          validations: [
            {
              name: 'KPI cards visible',
              selector: '[data-testid="kpi-card"], .kpi-card',
              type: 'exists'
            },
            {
              name: 'Revenue data displayed',
              selector: '.revenue, [data-testid="revenue"]',
              type: 'exists'
            }
          ]
        }
      ...
      steps: [
        {
          ...
          validations: [
            {
              name: 'Inventory items visible',
              selector: '[data-testid="inventory-item"], .inventory-item',
              type: 'exists'
            },
            {
              name: 'Stock status indicators',
              selector: '[data-status], .status-indicator',
              type: 'exists'
            }
          ]
        },
        {
          ...
          actions: [
            {
              type: 'verify',
              selector: '[data-status="low-stock"], .status-low-stock, .text-red-600'
            }
          ],
          validations: [
            {
              name: 'Low stock items identified',
              selector: '[data-status="low-stock"], .status-low-stock, .text-red-600',
              type: 'exists'
            }
          ]
          ...
          validations: [
            {
              ...
              selector: '[data-testid="heading-analytics"]',
              type: 'exists'
            }
          ]
        }
      ...
      steps: [
        {
          ...
          validations: [
            {
              name: 'Campaign list visible',
              selector: 'table, [data-testid="campaign-list"]',
              type: 'exists'
            },
            {
              name: 'Campaign metrics available',
              selector: '[data-testid="campaign-metrics"], .campaign-metrics',
              type: 'exists'
            }
          ]
        },
        {
          ...
          validations: [
            {
              name: 'Filter applied',
              selector: '[data-testid="campaign-status-filter"], select',
              type: 'exists'
            }
          ]
        },
        {
          ...
          module: 'marketing',
          actions: [{ type: 'verify', selector: '.chart, canvas, svg' }],
          validations: [
      ...
      steps: [
        {
          ...
          validations: [
            {
              name: 'Appointments displayed',
              selector: '[data-testid="appointment"], .appointment-item',
              type: 'exists'
            },
            {
              name: 'Staff assignments visible',
              selector: '.staff-name, [data-testid="staff-name"]',
              type: 'exists'
            }
          ]
        },
        ...
        },
        {
          ...
          validations: [
            {
              name: 'Staff performance data',
              selector: '.staff-performance, [data-testid="staff-performance"]',
              type: 'exists'
            }
          ]
        }
      ...
      steps: [
        {
          ...
          validations: [
            {
              name: 'KPI cards loaded',
              selector: '[data-testid="kpi-card"], .kpi-card',
              type: 'count',
              expected: { min: 3 }
            },
            {
              name: 'Revenue metrics visible',
              selector: '.revenue, [data-testid="revenue"]',
              type: 'exists'
            }
          ]
          ...
          validations: [
            {
              ...
              selector: 'button:has-text("Export"), button[aria-label*="Export"]',
              type: 'exists'
            }
          ]
        },
  ...
  private async executeWorkflowStep(
  ...
  ): Promise<UserWorkflowResult['workflow'][0]> {
    const startTime = Date.now()
    ...
    try {
      ...
      return {
        ...
        error:
          failedValidations > 0
            ? `${failedValidations}/${totalValidations} validations failed`
            : undefined
      }
    } catch (error) {
      return {
  ...
  private async runValidation(
    validation: BusinessWorkflow['steps'][0]['validations'][0]
  ): Promise<{ name: string; success: boolean; error?: string; details?: any }> {
    if (!this.page) throw new Error('Page not initialized')
          ...
          if (validation.selector && validation.expected) {
            ...
            if (element) {
              ...
              return {
                ...
                error: !matches
                  ? `Text mismatch: expected "${validation.expected}", got "${text}"`
                  : undefined
              }
            }
          }
        ...
        case 'count':
          if (validation.selector && validation.expected) {
            ...
            return {
              ...
              error: !success
                ? `Count validation failed: got ${count}, expected ${JSON.stringify(expected)}`
                : undefined
            }
          }
          break
          ...
          if (validation.selector && validation.expected) {
            ...
            if (element) {
              ...
              return {
                ...
                error: !matches
                  ? `Value mismatch: expected "${validation.expected}", got "${value}"`
                  : undefined
              }
            }
          }
          ...
          if (validation.selector && validation.expected) {
            ...
            if (element) {
              ...
              return {
                ...
                error: !matches
                  ? `Attribute mismatch: ${attr} expected "${expectedValue}", got "${actualValue}"`
                  : undefined
              }
            }
          }
