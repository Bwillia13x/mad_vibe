...
export class UserWorkflowTests {
  ...
  private async testDemoScenarioExecution(): Promise<WorkflowTestResult> {
    ...
    try {
      ...
      const demoControls = await this.page.$('.demo-controls, [class*="demo"], [data-testid*="demo"]');
      if (!demoControls) {
  ...
  private async testPOSWorkflow(): Promise<WorkflowTestResult> {
    ...
    try {
      ...
      const step5Start = Date.now();
      try {
        const regionSelects = await this.page.$$('select, [role="combobox"], button[role="combobox"]');
  ...
  private async testLoyaltyWorkflow(): Promise<WorkflowTestResult> {
    ...
    try {
      ...
      for (const button of addButtons) {
        const text = await button.evaluate(el => el.textContent);
        if (text?.includes('Add') && text.length < 10) { // Short "Add" button
      ...
      if (customerSelect && pointsInput && addPointsButton) {
        ...
        await this.delay(500);
        const firstCustomer = await this.page.$('[data-testid^="customer-select-"]:not([data-testid="customer-select-placeholder"])');
        if (firstCustomer) {
