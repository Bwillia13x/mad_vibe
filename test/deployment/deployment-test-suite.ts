...
import { DatabaseConnectivityTester, type DatabaseTestResult } from './database-connectivity-tests.js';
...
export class DeploymentTestRunner {
  ...
  private printSummary(results: DeploymentTestSuite): void {
    ...
    // Suite breakdown
    console.log(`\n📦 Docker Deployment Tests:`);
    console.log(`   ${summary.suites.docker.passed}/${summary.suites.docker.total} passed (${(summary.suites.docker.duration / 1000).toFixed(2)}s)`);

    console.log(`\n🗄️  Database Connectivity Tests:`);
    console.log(`   ${summary.suites.database.passed}/${summary.suites.database.total} passed (${(summary.suites.database.duration / 1000).toFixed(2)}s)`);

    console.log(`\n🏥 Health Monitoring Tests:`);
    console.log(`   ${summary.suites.health.passed}/${summary.suites.health.total} passed (${(summary.suites.health.duration / 1000).toFixed(2)}s)`);
