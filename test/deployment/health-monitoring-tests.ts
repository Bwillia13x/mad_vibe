  ...
  private async testHealthEndpointComprehensive(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        const inconsistentResponses = responses.filter(r => 
          JSON.stringify(Object.keys(r).sort()) !== JSON.stringify(firstKeys)
        );
    ...
    try {
      ...
      try {
        ...
        if (avgDuration > maxAcceptableAvg) {
          console.warn(`Average response time (${avgDuration}ms) exceeds threshold (${maxAcceptableAvg}ms)`);
        }
        
        if (p95Duration > maxAcceptableP95) {
          console.warn(`95th percentile response time (${p95Duration}ms) exceeds threshold (${maxAcceptableP95}ms)`);
        }
  ...
  private async testHealthMonitoringUnderLoad(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        const avgHealthDuration = successfulChecks.reduce((sum, c) => sum + c.duration, 0) / successfulChecks.length;
        ...
        if (healthSuccessRate < minHealthSuccessRate) {
          throw new Error(`Health monitoring success rate (${healthSuccessRate}) below threshold under load`);
        }
    ...
    try {
      ...
      try {
        ...
        if (failedEnvTests.length > 0) {
          throw new Error(`Environment validation failed: ${failedEnvTests.map(t => t.name).join(', ')}`);
        }
