...
export class BrowserCompatibilityTests {
  ...
  async runAllBrowserTests(): Promise<BrowserTestResult[]> {
    ...
    const browserConfigs = [
      ...
      { name: 'Chrome Mobile', args: ['--no-sandbox', '--disable-setuid-sandbox', '--user-agent=Mozilla/5.0 (iPhone; CPU iPhone OS 14_7_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/14.1.2 Mobile/15E148 Safari/604.1'] },
      { name: 'Chrome Tablet', args: ['--no-sandbox', '--disable-setuid-sandbox', '--user-agent=Mozilla/5.0 (iPad; CPU OS 14_7_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/14.1.2 Mobile/15E148 Safari/604.1'] }
    ];
  ...
  private async testBrowserCompatibility(browserName: string, launchArgs: string[]): Promise<BrowserTestResult> {
    ...
    try {
      ...
      const allTestsPassed = basicFunctionality && responsiveDesign && jsFeatures && 
                           cssFeatures && formSubmission && navigation;
  ...
  private async testNavigation(page: puppeteer.Page): Promise<boolean> {
    try {
      ...
      const navItems = [
        { button: '[data-testid="button-tool-scheduling"]', heading: '[data-testid="heading-scheduling"]' },
        { button: '[data-testid="button-tool-inventory"]', heading: '[data-testid="heading-inventory"]' },
        { button: '[data-testid="button-tool-analytics"]', heading: '[data-testid="heading-analytics"]' }
      ];
  ...
  private async measurePerformance(page: puppeteer.Page): Promise<BrowserTestResult['performance']> {
    try {
      // Measure load time
  ...
  private async testResponsiveViewport(viewport: { name: string; width: number; height: number }): Promise<ResponsiveTestResult> {
    const startTime = Date.now();
    let browser: puppeteer.Browser | null = null;
    let page: puppeteer.Page | null = null;
  ...
  private async testSidebarAdaptation(page: puppeteer.Page, viewportWidth: number): Promise<boolean> {
    try {
      ...
      if (viewportWidth < 768) {
        return sidebarInfo.width < 100 || sidebarInfo.position === 'fixed' || sidebarInfo.position === 'absolute';
      }
