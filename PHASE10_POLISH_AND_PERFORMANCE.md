...
export async function generateAgentResultPDF(
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.moveDown()
    doc.fontSize(10).text(`Completed: ${new Date(taskResult.completedAt!).toLocaleString()}`, { align: 'center' })
    doc.fontSize(10).text(`Duration: ${Math.round(taskResult.durationMs! / 1000)}s`, { align: 'center' })
