...
export async function getBusinessContext(): Promise<string> {
  try {
    ...
    const lowStockItems = inventoryItems.filter(item => 
      item.currentStock <= item.minStock && item.status !== 'out-of-stock'
    );
    ...
    const inProgressAppointments = todaysAppointments.filter(apt => 
...
**ACTIVE STAFF TODAY:**
${allStaff.filter(staff => staff.isActive).map(staff => 
  `- ${staff.name} (${staff.role}) - Specializes in: ${staff.specialties.join(', ')}`
).join('\n')}

**INVENTORY ALERTS:**
${lowStockItems.length > 0 ? `LOW STOCK (${lowStockItems.length} items):
${lowStockItems.map(item => `- ${item.name}: ${item.currentStock}/${item.minStock} units`).join('\n')}` : ''}
${outOfStockItems.length > 0 ? `OUT OF STOCK (${outOfStockItems.length} items):
...
${latestAnalytics ? `- Monthly Revenue: $${latestAnalytics.totalRevenue}
...
- No-Show Rate: ${(parseFloat(latestAnalytics.noShowRate) * 100).toFixed(1)}%` : 'No recent analytics available'}

**KEY INSIGHTS:**
- Total active customers: ${allCustomers.length}
- Popular services: ${(latestAnalytics?.topServices && Array.isArray(latestAnalytics.topServices)) ? latestAnalytics.topServices.slice(0, 3).map((s: any) => s.serviceName).join(', ') : 'Executive Cut, Skin Fade, Beard Sculpt'}
...
${appointments.map(apt => {
  ...
  const time = new Date(apt.scheduledStart).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
...
${alerts.map(item => 
  `- ${item.name}: ${item.currentStock}/${item.minStock} units (${item.status})`
).join('\n')}`;
...
${allStaff.filter(s => s.isActive).map(staff => 
  `- ${staff.name} (${staff.role}): ${staff.specialties.join(', ')} - ${staff.experience} years experience`
      ...
      case 'analytics':
        ...
        return latest ? `RECENT PERFORMANCE:
...
- Average Rating: ${latest.averageRating}/5.0` : 'No analytics available';
