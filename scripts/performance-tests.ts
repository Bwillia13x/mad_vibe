...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log('\n📈 Performance Metrics:');
    const loadTestResults = results.filter(r => r.testName.includes('load-test') && r.status === 'pass');
    
    if (loadTestResults.length > 0) {
      loadTestResults.forEach(result => {
        const loadResult = result.result?.loadTestResult || result.result?.spikeTestResult || result.result?.sustainedTestResult;
        if (loadResult) {
          console.log(`  • ${result.testName}:`);
          console.log(`    - Total requests: ${loadResult.summary.totalRequests}`);
          console.log(`    - Average response time: ${loadResult.summary.averageResponseTime.toFixed(0)}ms`);
    ...
    const responseTimeResults = results.filter(r => r.testName.includes('response-time') && r.status === 'pass');
    if (responseTimeResults.length > 0) {
      console.log('\n⏱️  Response Time Results:');
      responseTimeResults.forEach(result => {
        const rtResult = result.result;
        if (rtResult) {
          console.log(`  • ${rtResult.endpoint}: avg=${rtResult.avgResponseTime.toFixed(0)}ms, p95=${rtResult.p95ResponseTime.toFixed(0)}ms`);
        }
      });
    }
    
    // Display memory test results
    const memoryResults = results.filter(r => r.testName.includes('memory') && r.status === 'pass');
      ...
      memoryResults.forEach(result => {
        ...
        if (memResult) {
          if (memResult.memoryIncrease !== undefined) {
            console.log(`  • ${result.testName}: increase=${memResult.memoryIncrease}MB, max=${memResult.maxMemory}MB`);
          } else if (memResult.stable !== undefined) {
            console.log(`  • ${result.testName}: stable=${memResult.stable}, stddev=${memResult.memoryStdDev?.toFixed(1)}MB`);
          }
        }
      });
    }
    ...
    loadTestResults.forEach(result => {
      ...
      if (loadResult) {
        if (loadResult.summary.averageResponseTime > 100) {
          recommendations.push(`Consider optimizing response times (current avg: ${loadResult.summary.averageResponseTime.toFixed(0)}ms)`);
        }
        if (loadResult.summary.errorRate > 0.5) {
          recommendations.push(`Investigate error causes (current rate: ${loadResult.summary.errorRate.toFixed(1)}%)`);
        }
        if (loadResult.summary.throughput < 20) {
          recommendations.push(`Consider scaling improvements (current throughput: ${loadResult.summary.throughput.toFixed(1)} RPS)`);
        }
      }
    });
    ...
    memoryResults.forEach(result => {
      ...
      if (memResult?.memoryIncrease > 50) {
        recommendations.push(`Monitor for potential memory leaks (increase: ${memResult.memoryIncrease}MB)`);
      }
