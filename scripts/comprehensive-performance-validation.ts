...
class PerformanceValidator {
    ...
    try {
      ...
      try {
        ...
        if (loadTestResult.errorRate > this.requirements.maxErrorRate) {
          violations.push(`Error rate (${loadTestResult.errorRate.toFixed(2)}%) exceeds threshold (${this.requirements.maxErrorRate}%)`);
        }
        
        if (loadTestResult.maxConcurrentUsers < this.requirements.minConcurrentUsers) {
          violations.push(`Maximum concurrent users (${loadTestResult.maxConcurrentUsers}) below requirement (${this.requirements.minConcurrentUsers})`);
        }
        ...
        if (responseTimeResult.averageResponseTime > this.requirements.maxAverageResponseTime) {
          violations.push(`Average response time (${responseTimeResult.averageResponseTime.toFixed(1)}ms) exceeds threshold (${this.requirements.maxAverageResponseTime}ms)`);
        }
        
        if (responseTimeResult.p95ResponseTime > this.requirements.maxP95ResponseTime) {
          violations.push(`95th percentile response time (${responseTimeResult.p95ResponseTime.toFixed(1)}ms) exceeds threshold (${this.requirements.maxP95ResponseTime}ms)`);
        }
        ...
        if (sustainedLoadResult.throughput < this.requirements.minThroughput) {
          violations.push(`Sustained throughput (${sustainedLoadResult.throughput.toFixed(1)} RPS) below threshold (${this.requirements.minThroughput} RPS)`);
        }
        ...
        this.generateRecommendations(loadTestResult, responseTimeResult, sustainedLoadResult, memoryResult, recommendations);
  ...
  private async validateLoadTesting() {
    ...
    for (const userCount of userCounts) {
      ...
      try {
        ...
        console.log(`      ✅ ${userCount} users: ${result.errorRate.toFixed(2)}% error rate, ${result.averageResponseTime.toFixed(1)}ms avg response`);
  ...
  private async runLoadTest(concurrentUsers: number, durationMs: number): Promise<LoadTestResult> {
    ...
    const results: Array<{ success: boolean; responseTime: number; endpoint: string; status?: number }> = [];
  ...
  private async validateResponseTimes() {
    ...
    for (const endpoint of endpoints) {
      ...
      const passed = averageResponseTime <= this.requirements.maxAverageResponseTime && successCount > 0;
      ...
      console.log(`      ✅ ${averageResponseTime.toFixed(1)}ms avg (${successCount}/${iterations} success)`);
    }
  ...
  private async validateSpikeLoad() {
    ...
    console.log(`    ✅ Spike test: ${result.errorRate.toFixed(2)}% error rate with ${concurrentUsers} concurrent users`);
  ...
  private generateRecommendations(
    ...
    if (loadResult.errorRate > 0.5) {
      recommendations.push('Consider implementing connection pooling and request queuing to reduce error rates');
    }
    ...
    if (loadResult.maxConcurrentUsers >= this.requirements.minConcurrentUsers) {
      recommendations.push('Excellent concurrent user support - consider stress testing with even higher loads');
    }
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log(`Average Response Time: ${result.averageResponseTime.toFixed(1)}ms (< 200ms required)`);
