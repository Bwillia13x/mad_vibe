...
class ErrorRateAnalyzer {
  ...
  async analyzeErrorPatterns(): Promise<ErrorAnalysis> {
    ...
    try {
      ...
      console.log(`📊 Load test completed: ${result.summary.totalRequests} requests, ${result.summary.errorRate.toFixed(1)}% error rate`);
  ...
  private generateRecommendations(
    ...
    for (const [errorType, count] of errorsByType.entries()) {
      ...
      if (errorType.includes('ECONNRESET') && percentage > 10) {
        recommendations.push('High connection reset rate suggests server is dropping connections under load - implement connection pooling and keep-alive');
      }
      
      if (errorType.includes('ECONNREFUSED') && percentage > 5) {
        recommendations.push('Connection refused errors indicate server may be overwhelmed - implement request queuing or rate limiting');
      }
      
      if (errorType.includes('timeout') && percentage > 10) {
        recommendations.push('High timeout rate suggests slow response times - optimize database queries and add caching');
      }
      
      if (errorType.includes('Socket Hang Up') && percentage > 10) {
        recommendations.push('Socket hang up errors indicate premature connection closure - review session management and connection handling');
      }
      
      if (errorType.includes('EMFILE') || errorType.includes('ENFILE')) {
        recommendations.push('File descriptor limit reached - increase system limits and implement connection pooling');
      }
      
      if (errorType.includes('HTTP 500') && percentage > 5) {
        recommendations.push('Internal server errors detected - review error handling and add proper exception management');
      }
      
      if (errorType.includes('HTTP 503') && percentage > 5) {
        recommendations.push('Service unavailable errors suggest resource exhaustion - implement circuit breakers and graceful degradation');
      }
    }
    
    // Analyze endpoint-specific issues
    const sortedEndpoints = Array.from(errorsByEndpoint.entries())
      .sort((a, b) => b[1] - a[1]);
    ...
    if (sortedEndpoints.length > 0) {
      ...
      if (topErrorPercentage > 30) {
        recommendations.push(`Endpoint ${topErrorEndpoint} has ${topErrorPercentage.toFixed(1)}% of all errors - focus optimization efforts here`);
      }
    }
    
    // Analyze response time correlation
    if (result.summary.averageResponseTime > 100) {
      recommendations.push('High average response time correlates with error rate - optimize slow endpoints first');
    }
    
    // Analyze concurrency issues
    if (result.summary.errorRate > 3 && result.summary.concurrentUsers > 20) {
      recommendations.push('Error rate increases with concurrency - implement proper resource management and connection limits');
    }
    
    // General recommendations based on error rate
    if (result.summary.errorRate > 5) {
      recommendations.push('Critical error rate detected - implement comprehensive error handling and retry mechanisms');
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log('\n🎯 Errors by Endpoint:');
    const sortedEndpoints = Array.from(analysis.errorsByEndpoint.entries())
      .sort((a, b) => b[1] - a[1]);
    ...
    console.log('\n🔍 Errors by Type:');
    const sortedTypes = Array.from(analysis.errorsByType.entries())
      .sort((a, b) => b[1] - a[1]);
    ...
    const sortedWindows = Array.from(analysis.errorsByTimeWindow.entries())
      .sort((a, b) => b[1] - a[1]);
