...
async function main() {
  ...
  try {
    ...
    await delay(250)
    const invTextClosed = await page.$eval(toButton('inventory'), (el) =>
      (el.textContent || '').trim()
    )
    if (invTextClosed.length > 0)
      throw new Error('Sidebar closed but inventory label still visible')
    ...
    await page.waitForSelector('[data-testid="heading-loyalty"]', { timeout: 10000 })
    const invTextClosedReload = await page.$eval(toButton('inventory'), (el) =>
      (el.textContent || '').trim()
    )
    const sidebarClass = await page.$eval(
      '[data-testid="sidebar"]',
    ...
    if (invTextClosedReload.length > 0 || !/\bw-16\b/.test(sidebarClass)) {
      throw new Error(
        `Sidebar persisted state failed after reload (text='${invTextClosedReload}', class='${sidebarClass}', stored='${persisted}')`
      )
    }
    ...
    await page.click('[data-testid="button-toggle-sidebar"]')
    await delay(250)
    const invTextOpen = await page.$eval(toButton('inventory'), (el) =>
      (el.textContent || '').toLowerCase()
    )
    if (!invTextOpen.includes('inventory'))
      throw new Error('Sidebar open but inventory label missing')
...
main().catch((err) => {
  console.error('Navigation E2E failed:', err)
  process.exit(1)
})
