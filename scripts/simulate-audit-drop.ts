...
async function main() {
  ...
  try {
    ...
    const rows = await db
      .select()
      .from(aiAuditLogs)
      .where(and(eq(aiAuditLogs.capability, capability), gt(aiAuditLogs.createdAt, windowStart)))
    ...
    console.log(
      '[simulate-audit-drop] run `npm run sync:ai-audit-logs` to propagate drop scenario to warehouse'
    )
    console.log(
      '[simulate-audit-drop] restore timestamps with `tsx scripts/restore-audit-backup.ts --backup-file',
      backupPath,
      ' --capability',
      capability,
      '`'
