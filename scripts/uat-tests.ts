...
async function runUATTests() {
  ...
  console.log(
    '   This comprehensive test validates all demo scenarios, user workflows, and acceptance criteria'
  )
  ...
  try {
    ...
    for (const scenario of results.demoScenarios.results) {
      ...
      console.log(
        `   ${icon} ${scenario.name} (${scenario.scenario}) - ${(scenario.duration / 1000).toFixed(2)}s`
      )
    ...
    console.log(
      `   ${scriptIcon} ${results.demoScript.result.name} - ${(results.demoScript.result.duration / 1000).toFixed(2)}s`
    )
    ...
    console.log(
      `   ${keyboardIcon} ${results.keyboardShortcuts.result.name} - ${(results.keyboardShortcuts.result.duration / 1000).toFixed(2)}s`
    )
