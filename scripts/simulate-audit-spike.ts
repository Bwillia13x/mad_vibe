...
async function main() {
  ...
  try {
    ...
    const rows: InferInsertModel<typeof aiAuditLogs>[] = Array.from({ length: count }).map(
      (_, index) => {
    ...
    const inserted = await db
      .insert(aiAuditLogs)
      .values(rows)
      .returning({ id: aiAuditLogs.id, createdAt: aiAuditLogs.createdAt })
    ...
    console.log(
      '[simulate-audit-spike] run `npm run sync:ai-audit-logs` to push data to the warehouse'
    )
  } finally {
    await db.$client.end()
    await pool.end()
