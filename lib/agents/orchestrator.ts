...
export class AgentOrchestrator extends EventEmitter {
  ...
  private planAnalyze10K(params: Record<string, unknown>): AgentStep[] {
    ...
    return [
      this.createStep('step_1', 'Fetch Latest 10-K', `Download latest 10-K filing for ${ticker}`, 'fetch_filing', { ticker, formType: '10-K' }),
      this.createStep('step_2', 'Extract Financial Data', 'Parse income statement, balance sheet, cash flows', 'extract_financials', { sections: ['income_statement', 'balance_sheet', 'cash_flow'] }),
      this.createStep('step_3', 'Calculate Owner Earnings', 'Build owner earnings bridge with adjustments', 'calculate_owner_earnings'),
      this.createStep('step_4', 'Identify Key Metrics', 'Calculate ROIC, FCF yield, margins', 'calculate_metrics', { metrics: ['roic', 'fcf_yield', 'margins', 'growth_rates'] }),
      this.createStep('step_5', 'Extract MD&A Insights', 'Summarize management discussion and analysis', 'extract_mda'),
      this.createStep('step_6', 'Flag Red Flags', 'Identify accounting concerns and risks', 'identify_red_flags', { categories: ['accounting', 'governance', 'operations'] }),
      this.createStep('step_7', 'Generate Summary', 'Create comprehensive analysis summary', 'generate_summary')
    ]
  }
  ...
  private planBuildDCF(_params: Record<string, unknown>): AgentStep[] {
    return [
      this.createStep('step_1', 'Load Historical Financials', 'Gather 5 years of financial data', 'load_financials', { years: 5 }),
      this.createStep('step_2', 'Project Revenue', 'Forecast revenue growth for 10 years', 'project_revenue', { years: 10 }),
      this.createStep('step_3', 'Project Margins', 'Forecast operating and profit margins', 'project_margins'),
      this.createStep('step_4', 'Calculate WACC', 'Determine weighted average cost of capital', 'calculate_wacc'),
      this.createStep('step_5', 'Calculate Terminal Value', 'Determine terminal value using perpetuity growth', 'calculate_terminal_value', { method: 'perpetuity_growth' }),
      this.createStep('step_6', 'Discount Cash Flows', 'Calculate present value of FCF and terminal value', 'discount_cash_flows'),
      this.createStep('step_7', 'Run Sensitivity Analysis', 'Test valuation across WACC and growth assumptions', 'sensitivity_analysis')
    ]
  }
  ...
  private planCompetitiveAnalysis(params: Record<string, unknown>): AgentStep[] {
    ...
    return [
      this.createStep('step_1', 'Identify Competitors', 'Find top 5 competitors in same industry', 'identify_competitors', { count: 5 }),
      this.createStep('step_2', 'Fetch Competitor Data', `Load financial data for ${competitors.length || 5} competitors`, 'fetch_competitor_data', { competitors }),
      this.createStep('step_3', 'Compare Metrics', 'Compare ROIC, margins, growth rates', 'compare_metrics', { metrics: ['roic', 'margins', 'growth', 'valuation'] }),
      this.createStep('step_4', 'Analyze Competitive Position', 'Assess competitive advantages and disadvantages', 'analyze_position'),
      this.createStep('step_5', 'Generate Report', 'Create competitive analysis report', 'generate_report')
    ]
  }
  ...
  private planThesisValidation(_params: Record<string, unknown>): AgentStep[] {
    return [
      this.createStep('step_1', 'Extract Current Thesis', 'Load investment thesis from workspace', 'extract_thesis'),
      this.createStep('step_2', 'Gather Evidence', 'Collect supporting and contradicting data', 'gather_evidence'),
      this.createStep('step_3', 'Challenge Assumptions', 'Test key assumptions with data', 'challenge_assumptions'),
      this.createStep('step_4', 'Identify Weak Points', 'Find vulnerabilities in thesis', 'identify_weak_points'),
      this.createStep('step_5', 'Generate Validation Report', 'Create thesis strength assessment', 'generate_validation')
    ]
  }
  ...
  private planRiskAssessment(_params: Record<string, unknown>): AgentStep[] {
    return [
      this.createStep('step_1', 'Identify Risk Categories', 'Categorize operational, financial, market risks', 'categorize_risks'),
      this.createStep('step_2', 'Assess Probability', 'Estimate likelihood of each risk', 'assess_probability'),
      this.createStep('step_3', 'Calculate Impact', 'Quantify potential impact on value', 'calculate_impact'),
      this.createStep('step_4', 'Prioritize Risks', 'Rank risks by probability × impact', 'prioritize_risks')
    ]
  }
  ...
  private planQuarterlyUpdate(_params: Record<string, unknown>): AgentStep[] {
    return [
      this.createStep('step_1', 'Fetch Latest 10-Q', 'Download most recent quarterly filing', 'fetch_filing', { formType: '10-Q' }),
      this.createStep('step_2', 'Compare to Expectations', 'Check results vs guidance and estimates', 'compare_expectations'),
      this.createStep('step_3', 'Update Financial Model', 'Revise projections based on actuals', 'update_model'),
      this.createStep('step_4', 'Reassess Thesis', 'Determine if thesis still valid', 'reassess_thesis'),
      this.createStep('step_5', 'Generate Update Report', 'Create quarterly update summary', 'generate_update')
    ]
  }
