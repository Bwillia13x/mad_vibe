...
export class ResourceManager {
  ...
  private updateMetrics(): void {
    ...
    this.metrics.gc = {
      ...
      averageGCTime: this.gcStats.gcTimes.length > 0 
    ...
    this.metrics.resources = {
      activeTimers: (process as any)._getActiveHandles?.()?.filter((h: any) => h.constructor.name === 'Timeout').length || 0,
  ...
  private cleanupExpiredCache(): void {
    ...
    if (cleanedCount > 0) {
      log('Cleaned up expired cache entries', {
        cleanedCount,
  ...
  clearCache(): void {
    ...
    this.cache.clear();
    log('Cache cleared', { entriesRemoved: size });
  }
  ...
  getStatus(): {
    ...
    return {
      healthy: metrics.memory.heapUtilization < 85 && 
