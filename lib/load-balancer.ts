...
export class LoadBalancer {
   ...
   */
  addInstance(instance: Omit<ServerInstance, 'healthy' | 'connections' | 'lastHealthCheck' | 'responseTime' | 'errorCount' | 'totalRequests'>): void {
  ...
  private updateMetrics(): void {
    ...
    if (healthyInstances.length > 0) {
      this.metrics.averageResponseTime = healthyInstances.reduce((sum, i) => sum + i.responseTime, 0) / healthyInstances.length;
  ...
  private checkAutoScaling(): void {
    ...
    if (this.metrics.currentLoad > this.config.scaleUpThreshold &&
    ...
    else if (this.metrics.currentLoad < this.config.scaleDownThreshold &&
